...
const el = {
  ...
  statsAnalyzeCount: document.getElementById('stats-analyze-count'),
  statsCertificateCount: document.getElementById('stats-certificate-count'),
...
async function refreshStats() {
  const counters = await window.HandDB.getCounters();
  el.statsAnalyzeCount.textContent = counters.analyzeCount + ' 回';
  el.statsCertificateCount.textContent = counters.certificateCount + ' 回';
}
...
el.btnAnalyze.addEventListener('click', async () => {
  ...
  try {
    ...
    showResult(result);
    el.analyzeStatus.textContent = '';
    await window.HandDB.incrementCounter('analyzeCount');
    await refreshStats();
  } catch (err) {
    console.error(err);
...
// ================= CERTIFICATE =================
el.btnCertificate.addEventListener('click', async () => {
  ...
  await window.HandDB.incrementCounter('certificateCount');
  await refreshStats();
});
